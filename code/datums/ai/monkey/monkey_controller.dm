...
/datum/ai_controller/monkey/pun_pun/TryPossessPawn(atom/new_pawn)
	...
	// set_blackboard_key(BB_MONKEY_PRESS_TYPEPATH, /obj/structure/desk_bell) Похуй потом
...
/datum/ai_controller/monkey/get_able_to_run()
	...
	if(INCAPACITATED_IGNORING(living_pawn, INCAPABLE_RESTRAINTS|INCAPABLE_GRAB) || living_pawn.stat > CONSCIOUS)
		return AI_UNABLE_TO_RUN
	return ..()

/datum/ai_controller/monkey/proc/set_trip_mode(mode = TRUE)
	var/mob/living/carbon/regressed_monkey = pawn
	var/brain = regressed_monkey.get_organ_slot(INTERNAL_ORGAN_BRAIN)
...
/datum/ai_controller/monkey/proc/TryFindWeapon()
	...
	if(!(locate(/obj/item) in living_pawn.get_held_items()))
		set_blackboard_key(BB_MONKEY_BEST_FORCE_FOUND, 0)

	if(blackboard[BB_MONKEY_GUN_NEURONS_ACTIVATED] && (locate(/obj/item/gun) in living_pawn.get_held_items()))
	...
	for(var/obj/item/item in living_pawn.get_held_items()) // If we've got some garbage in out hands that's going to stop us from effectively attacking, we should get rid of it.
		if(item.force < 2)
			living_pawn.drop_item_ground(item)

	weapon = GetBestWeapon(src, nearby_items, living_pawn.get_held_items())

	var/pickpocket = FALSE
	for(var/mob/living/carbon/human/human in oview(5, living_pawn))
		var/obj/item/held_weapon = GetBestWeapon(src, human.get_held_items() + weapon, living_pawn.get_held_items())
		if(held_weapon == weapon) // It's just the same one, not a held one
			continue
		pickpocket = TRUE
		weapon = held_weapon

	if(!weapon || (weapon in living_pawn.get_held_items()))
		return FALSE
