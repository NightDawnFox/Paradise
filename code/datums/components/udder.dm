...
/obj/item/udder
	...
	var/reagent_produced_typepath = /datum/reagent/consumable/drink/milk
...
/obj/item/udder/proc/handle_consumption(atom/movable/food, mob/user)
	...
	if(isstack(food)) //if stack, only consume 1
		...
		final_food = food_stack.split(user, 1)
...
/obj/item/udder/gutlunch
	...
	reagent_produced_typepath = /datum/reagent/bugmilk
...
/obj/item/udder/gutlunch/generate()
	...
	if(locate(/obj/item/stack/ore/gold) in src)
		reagents.add_reagent(/datum/reagent/consumable/drink/milk/cream, rand(2, 5))
...
/obj/item/udder/raptor/generate()
	...
	if(prob(happiness_percentage))
		reagents.add_reagent(/datum/reagent/consumable/drink/milk/cream, 5)

	var/minimum_bound = happiness_percentage > 0.6 ? 10 : 5
	var/upper_bound = minimum_bound + 5
	reagents.add_reagent(reagent_produced_typepath, rand(minimum_bound, upper_bound))
