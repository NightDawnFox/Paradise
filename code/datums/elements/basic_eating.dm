...
/datum/element/basic_eating/proc/try_feed(atom/source, atom/possible_food, mob/living/user)
	SIGNAL_HANDLER
	if(user.intent != INTENT_HELP || !is_type_in_list(possible_food, food_types))
		return NONE
	...
	return try_eating(source, possible_food, user) ? COMPONENT_CANCEL_ATTACK_CHAIN|COMPONENT_NO_AFTERATTACK : NONE
...
/datum/element/basic_eating/proc/finish_eating(mob/living/eater, atom/target, mob/living/feeder, to_heal)
	...
	// eater.log_message("has eaten [target], [devour ? "swallowing it" : "destroying it"]!", LOG_ATTACK) Похуй потом
