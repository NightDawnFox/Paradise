/datum/element/wears_collar // Похуй потом
...
/datum/element/wears_collar/proc/on_content_exit(mob/living/source, atom/moved)
	...
	if(!istype(moved, /obj/item/clothing/accessory/petcollar))
		return
	// source.fully_replace_character_name(null, source::name) // Похуй потом
	if(collar_icon_state)
		source.update_appearance()

/datum/element/wears_collar/proc/on_content_enter(mob/living/source, obj/item/clothing/accessory/petcollar/new_collar)
	SIGNAL_HANDLER

	if(!istype(new_collar) || !new_collar.tagname)
		return

	// source.fully_replace_character_name(null, "\proper [new_collar.tagname]") // Похуй потом
	if(collar_icon_state)
		source.update_appearance()
...
/datum/element/wears_collar/proc/attach_collar(atom/source, atom/movable/attacking_item, atom/user, params)
	...
	if(!istype(attacking_item, /obj/item/clothing/accessory/petcollar))
		return NONE

	if(locate(/obj/item/clothing/accessory/petcollar) in source)
