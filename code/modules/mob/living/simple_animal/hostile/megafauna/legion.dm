...
/mob/living/simple_animal/hostile/megafauna/legion/OpenFire(the_target)
	if(world.time >= ranged_cooldown && !charging)
		if(prob(30))
			...
			Beam(target_location, icon_state = "death_laser", time = beam_time, maxdistance = INFINITY, beam_type = /obj/effect/ebeam/disintegration_telegraph, layer = ON_EDGED_TURF_LAYER)
...
/mob/living/simple_animal/hostile/megafauna/legion/proc/fire_disintegration_laser(location)
	...
	Beam(location, icon_state = "death_laser", time = 2 SECONDS, maxdistance = INFINITY, beam_type = /obj/effect/ebeam/reacting/disintegration, layer = ON_EDGED_TURF_LAYER)
